<template>
  <dx-data-grid
    ...
    :data-source="vars.measureRequirement"
    :show-borders="true"
  ...
  >
    <dx-column caption="소요량 산출번호" data-field="number" :sort-index="1" sort-order="desc" />
    <dx-column caption="일자" data-field="target_date"  data-type="date" format="yyyy-MM-dd" :sort-index="0" sort-order="desc" />
...
import { getMeasureRequirement } from '../../data-source/produce';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const methods = {
      refresh: async () => {
        vars.measureRequirement = getMeasureRequirement(vars.filter);
      },
