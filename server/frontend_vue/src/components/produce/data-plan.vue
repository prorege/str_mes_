<template>
  <dx-data-grid
    ...
    :data-source="vars.productionPlan"
    :show-borders="true"
  ...
  >
    <dx-column caption="생산계획번호" data-field="number" :sort-index="1" sort-order="desc" />
    <dx-column caption="생산계획일자" data-field="target_date" data-type="date" format="yyyy-MM-dd" :sort-index="0" sort-order="desc" />
...
import { getProducePlan } from '../../data-source/produce';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const methods = {
      refresh: async () => {
        vars.productionPlan = getProducePlan(vars.planFilter);
      },
