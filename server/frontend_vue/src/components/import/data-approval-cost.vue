<template>
  <dx-data-grid
    ...
    :data-source="importApprovalCost"
    :show-borders="true"
  ...
  >
    <dx-column caption="비용번호" data-field="approval_cost_number" :sort-index="1" sort-order="desc" />
    <dx-column caption="발행일" data-field="approval_cost_date" data-type="date" format="yyyy-MM-dd" :sort-index="0" sort-order="desc" />
    <dx-column caption="InvoiceNo" data-field="shipment.shipment_number" />
...
import { importApprovalCost } from '../../data-source/import';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    if (props.fixedFilter) {
      !(() => (importApprovalCost.defaultFilters = props.fixedFilter))();
    }
