<template>
  <dx-data-grid
    ...
    @initialized="(evt) => bindStateStore(evt)"
    @selection-changed="onSelectionChanged"
  >
    <dx-column caption="가입고번호" data-field="prereceiving_number" min-width="180" :sort-index="1" sort-order="desc" />
    <dx-column caption="가입고일자" data-field="prereceiving_date" data-type="date" format="yyyy-MM-dd" :sort-index="0" sort-order="desc" />
    <dx-column caption="공급업체" data-field="client_company" />
    <dx-column caption="업체담당자" data-field="client_manager" />
    <dx-column caption="입고부서" data-field="receiving_department" />
    <dx-column caption="입고담당자" data-field="receiving_manager" />
    <dx-column caption="입고구분" data-field="receiving_type" />
    ...
    <dx-column caption="무상수리기간" data-field="ref_number" />
    <dx-column caption="프로젝트" data-field="project_management.project_number" />
    ...
    <dx-column caption="입고확정" data-field="confirmed" />
...
import { purchasePreReceiving } from '../../data-source/purchase';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function bindStateStore (evt) {
      stateStore.bind('$prereceiving', evt.component);
    }
