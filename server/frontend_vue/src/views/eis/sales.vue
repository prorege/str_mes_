...
import {shipmentRelease, shipmentReleaseItem} from '@/data-source/shipment'
...
async function submit () {
  ...
  const {data, totalCount} = await shipmentReleaseItem.load()
  if (totalCount) {

    ds.series1.value = chain(data)
      .groupBy(item => item.release.client_company)
    ...
    ds.series2.value = chain(data)
      .groupBy(item => item.release.release_department)
      .transform((result, value, key) => {
        result.push({ key, value: value.reduce((r, item) => r + item.supply_price, 0) })
      }, [])
      .value()
      ...
      <div class="content-header">
        <dx-toolbar>
          <dx-item location="before">
            <div class="content-title">매출실적</div>
          </dx-item>
        </dx-toolbar>
      </div>
