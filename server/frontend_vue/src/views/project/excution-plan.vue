      ...
      async saveItem() {
        ...
        try {
          ...
          if (vars.formData.id) {
            ...
            if(updateDate.project_management){
              updateDate.project_management.company_amount = updateDate.contract_amount;
            }

            const { data } = await projectExcutionPlan.update(
              vars.formData.id,
              updateDate
            );
            ...
            if (planSubcontract && planSubcontract.hasEditData()) {
              await planSubcontract.saveEditData();
            }
            if (planExpense && planExpense.hasEditData()) {
              await planExpense.saveEditData();
            }
            await methods.approvalRequest();
            methods.redirect(data.id);
            vars.formState.readOnly = true;
