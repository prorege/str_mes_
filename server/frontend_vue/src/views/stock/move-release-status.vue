  ...
  <div>
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">재고이동출고현황</div>
            </dx-item>
          </dx-toolbar>
        </div>
      ...
      <div class="dx-card responsive-paddings mt-1">
        ...
        >
          <dx-column caption="출고번호" data-field="stock_move_release.number" :filter-operations="['contains', '=']" />
          <dx-column caption="출고일자" data-field="stock_move_release.target_date" data-type="date" format="yyyy-MM-dd" :show-in-column-chooser="false" />
          <dx-column caption="담당부서" data-field="stock_move_release.department" />
          <dx-column caption="담당자" data-field="stock_move_release.manager" />
          ...
          <dx-column caption="단가" data-field="unit_price" format="₩,##0" />
          <dx-column caption="단위" data-field="item.unit" />
          <dx-column caption="금액" data-field="supply_price" format="₩,##0" :calculate-cell-value="methods.calcSupplyPrice" />
          <dx-column caption="주공급업체" data-field="client_alias" />
...
import { stockMoveReleaseItem } from '../../data-source/stock';
  ...
  setup() {
    ...
    const methods = {
      onGridInitialized(evt, key) {
        ...
        stateStore.bind(`stock-move-release-${key}`, evt.component);
      ...
      initSorting() {
        ...
        if (col.length == 0) {
          const defaultName = 'stock_move_release.target_date';
      ...
      goDetail({ data }) {
        router.push({ path: `/stock/move-release/${data.stock_move_release.id}` });
      },
      calcSupplyPrice(rowData) {
        ...
        return { 
          filter: [
            [
              'stock_move_release.target_date', '>=',
          ...
          sort: [
            {selector : 'stock_move_release.target_date', desc: true}
          ],
          skip: 0,
          take: 10000
        }
      ...
      async searchDateRange() {
        ...
        // const dateFilter = filter.find(v => typeof v === 'object' && v[0] === 'stock_move_release.target_date')
        // if (dateFilter) dateFilter[2] = filterValue
        // else filter.push(['stock_move_release.target_date', 'between', filterValue])
        ...
        try{
          ...
          const { data } = await stockMoveReleaseItem.load(params);
      ...
      async onExporting(evt) {
        stockMoveReleaseItem.exportData(evt.component, '재고이동출고현황', `재고이동출고현황_${Date.now()}.xlsx`);
        evt.cancel = true;
      },
    };

    return { vars, methods, stockMoveReleaseItem, dataSource };
  },
};
</script>
