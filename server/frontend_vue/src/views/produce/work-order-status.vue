    ...
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">작업지시현황</div>
            </dx-item>
          </dx-toolbar>
        </div>
      ...
      <div class="dx-card responsive-paddings mt-1">
        ...
        >
          ...
          <dx-column caption="일자" data-field="work_order.target_date" data-type="date" format="yyyy-MM-dd" :show-in-column-chooser="false" />
          ...
          <dx-column caption="단가" data-field="item.purchase_price" format="₩,##0" />
...
export default {
  ...
  setup() {
    ...
    const methods = {
      ...
      goDetail({ data }) {
        router.push({ path: `/produce/work-order/${data.work_order.id}` });
      },
        getParams () {
        return { 
          filter: [
      ...
      onExporting (evt) {
        produceWorkOrderItem1.exportData(evt.component, '작업지시현황', `작업지시현황_${Date.now()}.xlsx`)
        evt.cancel = true
      }
    };
