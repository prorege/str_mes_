  ...
  <div>
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">생산계획현황</div>
            </dx-item>
          </dx-toolbar>
        </div>
      ...
      <div class="dx-card responsive-paddings mt-1">
        ...
        >
          <dx-column caption="생산계획번호" data-field="production_plan.number" :filter-operations="['contains', '=']" />
          <dx-column caption="일자" data-field="production_plan.target_date" data-type="date" format="yyyy-MM-dd" :show-in-column-chooser="false" />
          ...
          <dx-column caption="담당자" data-field="production_plan.manager" />
          ...
          <dx-column caption="공급가" data-field="supply_price" format="₩,##0" />
...
import { producePlanItem } from '../../data-source/produce';
...
export default {
  ...
  setup() {
    ...
    const methods = {
      ...
      goDetail({ data }) {
        router.push({ path: `/produce/plan/${data.production_plan.id}` });
      },
        getParams () {
        return { 
          filter: [
            [
              'production_plan.target_date', '>=',
          ...
          sort: [
            {selector : 'production_plan.target_date', desc: true}
          ],
          skip: 0,
          take: 10000
        }
      ...
      async searchDateRange() {
        ...
        // const dateFilter = filter.find(v => typeof v === 'object' && v[0] === 'production_plan.target_date')
        // if (dateFilter) dateFilter[2] = filterValue
        // else filter.push(['production_plan.target_date', 'between', filterValue])
        ...
        try{
          ...
          const { data } = await producePlanItem.load(params);
      ...
      onExporting (evt) {
        producePlanItem.exportData(evt.component, '생산계획현황', `생산계획현황_${Date.now()}.xlsx`)
        evt.cancel = true
      }
    };

    return { vars, methods, producePlanItem, dataSource };
  },
};
</script>
