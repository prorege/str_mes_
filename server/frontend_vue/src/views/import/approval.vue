  ...
  <div v-if="vars.init.value">
      ...
      <div class="dx-card responsive-paddings back-colored">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">결재등록</div>
            </dx-item>
        ...
        <dx-form :form-data="vars.formData">
          <dx-group-item :col-count="5">
            <dx-group-item>
              <dx-simple-item data-field="approval_number"
                :editor-options="{
                  placeholder: '(자동 or 직접입력)',
                  ...generateItemButtonOption('search', methods.createFindPopupFn('approval', '결재조회')),
                }"
              >
                <dx-label text="결재번호" :show-colon="false" />
              </dx-simple-item>
              <dx-simple-item data-field="approval_date" editor-type="dxDateBox"
                :editor-options="{
                  dateSerializationFormat: 'yyyy-MM-ddTHH:mm:ss',
                  ...vars.formState,
                }"
              >
                <dx-label text="결재일" :show-colon="false" />
              </dx-simple-item>
            </dx-group-item>
            <dx-group-item>
            ...
            <dx-group-item>
              ...
              >
                <dx-label text="환종" :show-colon="false" />
              </dx-simple-item>
      ...
      <div class="dx-card responsive-paddings mt-1">
        <div class="mt-2">
          <dx-data-grid
            ...
            :data-source="vars.dataSource.approvalItem"
            :on-initialized="(evt) => methods.onGridInitialized(evt, 'approvalItem')"
          ...
          >
            ...
            <dx-column caption="계산서번호" data-field="credit.credit_number" width="180" :allow-editing="false" />
            <dx-column caption="입고번호" data-field="shipment.shipment_number" :allow-editing="false" />
    ...
    >
      ...
      <template #popup-content>
        ...
        >
          <dx-column caption="Credit No." data-field="credit.credit_number" :sort-index="1" sort-order="desc" />
          <dx-column caption="Work Day" data-field="credit.work_day" :sort-index="0" sort-order="desc" />
          <dx-column caption="Invoice No." data-field="shipment.shipment_number" />
          <dx-column caption="Ship Date" data-field="shipment.shipment_date" />
          <dx-column caption="Amount" data-field="amount" data-type="number" :format="{ type: 'fixedPoint', precision: 2 }" />
    ...
    >
      <template #popup-content>
        ...
        <data-grid-approval   v-else-if="vars.dlg.finder.key === 'approval'" @change="methods.finderReturnHandler" />
      </template>
    </dx-popup>
...
import DataGridApproval from '../../components/import/data-approval.vue';

import { baseClient, baseCodeLoader } from '../../data-source/base';
import { importApproval, getImportApprovalItem, getImportCreditItem } from '../../data-source/import';
...
export default {
  components: {
    ...
    DataGridApproval, DataGridClient,
  },
  props: {
    id: [String, Number],
  },
  setup(props) {
    ...
    vars.grid = { creditItem: null, approvalItem: null };
    ...
    vars.warehouse = {};
    vars.filter = {};
    vars.filter.creditItem = { supplier: '' };
    vars.filter.approvalItem = [{ name: 'fk_import_approval_id', op: 'eq', val: props.id || 0 }];
    vars.dataSource = reactive({
      ...
      department: [],
      creditItem: null,
      approvalItem: getImportApprovalItem(vars.filter.approvalItem),
    });
    ...
    vars.focus = reactive({ approvalItem: null });
    ...
    const methods = {
      async initById(id) {
        await methods.refreshGridApprovalItem(id);
        if (!id) {
      ...
      clearFormData() {
        ...
        vars.formData.approval_number = ''; // 결재번호
        vars.formData.approval_date = ''; // 결재일자
        vars.formData.supplier = ''; // 공급업체
        vars.formData.department = ''; // 담당부서
        vars.formData.member = ''; // 담당자
        vars.formData.approval_type = ''; // 결재유형
      ...
      async addSelectedRows() {
        const grid = vars.grid.approvalItem;
        ...
        const rows = await vars.grid.creditItem.getSelectedRowsData();
        for (let row of rows) {
          console.log(row);
          await grid.addRow();
          const data = await grid.byKey(grid.getKeyByRowIndex(0));
          data.amount = row.amount; // 금액
          data.remark = ''; // 비고
          data.briefs = ''; // 적요
          data.fk_import_shipment_id = row.fk_import_shipment_id; // 입고 ID
          data.fk_import_credit_id = row.fk_import_credit_id; // 계산서 ID
          data.fk_import_credit_item_id = row.id; // 계산서 품목 ID
          data.fk_import_approval_id = vars.formData.id; // 발주 ID
      ...
      async refreshGridApprovalItem(id) {
        if (!id) { id = 0; }
        vars.filter.approvalItem[0].val = id;
        vars.dataSource.approvalItem.defaultFilters = vars.filter.approvalItem;
        if (vars.grid.approvalItem) {
          methods.clearGrid(vars.grid.approvalItem);
        }
      },
      onGridInitialized(evt, key) {
        vars.grid[key] = evt.component;
        stateStore.bind(`import-approval-${key}`, evt.component);
      },
      async newItem() {
        methods.refreshGridApprovalItem();
        if (vars.formData.id) {
          methods.redirect();
          vars.formData.id = null;
        }
        setTimeout(() => {
          vars.formData.created = null;
          vars.formData.approval_date = currentDateTime();
      ...
      async loadFormData(id) {
        let { data } = await importApproval.byKey(id);
        Object.assign(vars.formData, data);
      },
      async insertFormData(insertData) {
        return await importApproval.insert(insertData);
      },
      async updateFormData(id, updateData) {
        await importApproval.update(id, updateData);
      },
      async removeFormData(id) {
        await importApproval.remove(id);
      },
      async saveItem() {
        ...
        try {
          if (vars.formData.id) {
            // 기존 정보 업데이트
            await methods.updateFormData(vars.formData.id, vars.formData);
            await methods.saveGridApprovalItem();
            ...
            // 채번이 없을 경우 자동 채번
            let { data } = await methods.insertFormData(vars.formData);
      ...
      async saveGridApprovalItem() {
        await methods.saveGridItem(vars.grid.approvalItem);
      },
      ...
      async onSupplierChanged() {
        ...
        methods.setCreditItemFilter(vars.formData.supplier);
        methods.loadCreditItem();
      ...
      onSavingItem(e) {
        e.changes.forEach((element) => {
          if (element.type != 'remove') {
            element.data.fk_import_approval_id = vars.formData.id;
            delete element.data.approval;
            delete element.data.shipment;
            delete element.data.credit;
            delete element.data.credit_item;
          }
        });
        methods.saveSummary();
      },
      ...
      loadCreditItem() {
        vars.dataSource.creditItem = getImportCreditItem([
          {
            name: 'credit',
            op: 'has',
            val: {
              name: 'supplier',
              op: 'eq',
              val: vars.filter.creditItem.supplier,
            },
          },
        ]);
      },
      ...
      getFirstApprovalType() {
        return getFirstCodeNameInBaseCodeList(vars.dataSource.approval_type);
      },
      redirect(id) {
        let path = '/import/approval';
        if (id) { path = `${path}/${id}` }
      ...
      async printDocument() {
        ...
        const { data: item1 } = await vars.dataSource.approvalItem.load();
        params.items = [...item1];
      ...
      setCreditItemFilter(clientName) {
        vars.filter.creditItem.supplier = clientName;
      },
      decideDisableDelete() {
