        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">품목별 마감재고</div>
            </dx-item>
          </dx-toolbar>
        </div>
...
async function searchDateRange() {
  try {
    ...
    const grouped = groupBy(data.objects, 'item_code')
    ...
    Object.keys(grouped).forEach((key, i) => {
      result.push({
        id: i + 1,
        item_code: key,
        main_category: grouped[key][0].main_category,
        item_name: grouped[key][0].item_name,
        item_standard: grouped[key][0].item_standard,
