    ...
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">원가 재고수불장</div>
            </dx-item>
          </dx-toolbar>
        </div>
      ...
      <div class="dx-card responsive-paddings mt-1">
        ...
        >
          ...
          <template #print>
            <dx-button icon="print" text="원가상품수불장 출력" @click="methods.printProductReceipt" />
          </template>
          ...
          <dx-column caption="누적입고금액" data-field="accumulate_receiving_price" data-type="number" format="₩,##0.0000" :visible="false" />
        ...
        <div class="mt-2">
          <table class="summary-table-wide">
            <tr>
              ...
              <th>입고금액</th>
              <td>{{ vars.dataSource.sum.receiving_price }}</td>
              <th>출고수량</th>
              <td>{{ vars.dataSource.sum.release_stock }}</td>
              <th>출고원가금액</th>
              <td>{{ vars.dataSource.sum.release_price }}</td>
              <th>누적재고</th>
              <td>{{ vars.dataSource.sum.sum_by_case }}</td>
              <th>누적재고금액</th>
              <td>{{ vars.dataSource.sum.sum_by_price }}</td>
            </tr>
          </table>
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    vars.api = {
      statistics: new ApiService('/api/mes/v1/cost/statistics')
    }
    vars.detailItem = ref()
    ...
    onMounted(async () => {
      ...
      vars.dataSource.warehouse = vars.dataSource.warehouse.filter(warehouse => warehouse.use_cost_closing)
    ...
    const methods = {
      ...
      async searchDateRange() {
        ...
        try {
          ...
          response.objects.forEach(row => {
            ...
            sumReceivingPrice = sumReceivingPrice + parseFloat(row.receiving_price);
            row.accumulate_receiving_price = sumReceivingPrice;
            sumReleasePrice = sumReleasePrice + parseFloat(row.release_price);
            row.accumulate_release_price = sumReleasePrice;
          });
          vars.dataSource.status = response.objects;
          vars.dataSource.sum.past_stock = numeral(response.objects.reduce((sum, row) => sum + parseInt(row.past_stock, 10), 0)).format('0,0');
          vars.dataSource.sum.receiving_stock = numeral(response.objects.reduce((sum, row) => sum + parseInt(row.receiving_stock, 10), 0)).format('0,0');
          vars.dataSource.sum.release_stock = numeral(response.objects.reduce((sum, row) => sum + parseInt(row.release_stock, 10), 0)).format('0,0');
          vars.dataSource.sum.sum_by_case = numeral(sumByCase).format('0,0');

          vars.dataSource.sum.past_price = numeral(response.objects.reduce((sum, row) => sum + parseFloat(row.past_price, 10), 0)).format('0,0.0000');
          vars.dataSource.sum.receiving_price = numeral(response.objects.reduce((sum, row) => sum + parseFloat(row.receiving_price, 10), 0)).format('0,0.0000');
          vars.dataSource.sum.release_price = numeral(response.objects.reduce((sum, row) => sum + parseFloat(row.release_price, 10), 0)).format('0,0.0000');
          vars.dataSource.sum.sum_by_price = numeral(sumByPrice).format('0,0.0000');
        }
        catch (ex) {
          console.error(ex)
        }
      ...
      async printProductReceipt() {
        ...
        const acc = { 
          input: 0, output: 0, stock: parseInt(list[0].past_stock || 0),
          past_stock: 0, receiving_price: 0, release_price: 0, sum_by_price: 0
        }
        ...
        for(const year in years){
          for(const month of years[year]){
            ...
            item.past_stock = item.list.reduce((a, i) => a += parseFloat(i.past_stock), 0)
            item.receiving_price = item.list.reduce((a, i) => a += parseFloat(i.receiving_price), 0)
            item.release_price = item.list.reduce((a, i) => a += parseFloat(i.release_price), 0)
            item.sum_by_price = item.list[item.list.length - 1].sum_by_price

            acc.input += item.mon_input
            acc.output += item.mon_output
            acc.stock = acc.stock + item.mon_input - item.mon_output

            acc.past_stock += item.past_stock
            acc.receiving_price += item.receiving_price
            acc.release_price += item.release_price
            acc.sum_by_price = item.sum_by_price

            item.acc_past_stock = acc.past_stock
            item.acc_receiving_price = acc.receiving_price
            item.acc_release_price = acc.release_price
            item.acc_sum_by_price = acc.sum_by_price
        ...
        console.log(params)
        await printDocument('costreceiptsummary', params);
      },
    };
