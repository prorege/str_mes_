<template>
  <div v-if="vars.init.value">
    ...
    <div class="content-block">
      <div class="dx-card responsive-paddings back-colored">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">입금</div>
            </dx-item>
        ...
        <dx-form :form-data="vars.formData">
          <dx-group-item :col-count="5">
            <dx-group-item>
              <dx-simple-item
                data-field="deposit_number"
                :editor-options="{
                  placeholder: '(자동 or 직접입력)',
                  ...generateItemButtonOption('search', methods.createFindPopupFn('deposit', '입금조회')),
                }"
              >
                <dx-label text="입금번호" :show-colon="false" />
              </dx-simple-item>
              <dx-simple-item data-field="deposit_date" editor-type="dxDateBox"
                :editor-options="{ dateSerializationFormat: 'yyyy-MM-ddTHH:mm:ss', ...vars.formState }"
              >
                <dx-label text="입금일자" :show-colon="false" />
              </dx-simple-item>
            </dx-group-item>
            <dx-group-item>
            ...
            <dx-group-item>
              <dx-simple-item data-field="deposit_department" editor-type="dxSelectBox"
                :editor-options="{
              ...
              <dx-simple-item data-field="deposit_manager" editor-type="dxSelectBox"
                :editor-options="{
              ...
              >
                <dx-label text="영업담당자" :show-colon="false" />
              </dx-simple-item>
            </dx-group-item>
            <dx-group-item>
      ...
      <div class="dx-card responsive-paddings mt-1">
        <div class="mt-2">
          ...
          >
            <dx-grid-toolbar>
              <dx-item template="addFromSales" location="before" :visible="vars.isAllowEdit.value" />
            ...
            <template #addFromSales>
              <dx-button text="계산서에서 가져오기" icon="add" @click="methods.showAddPopup" />
            </template>

            <dx-column caption="계산서번호" data-field="sales.sales_number" width="180" :allow-editing="false" />
            <dx-column caption="금액" data-field="price" data-type="number" format="currency" :allow-editing="true" :set-cell-value="methods.setPrice" />
            <dx-column caption="입금형태" data-field="deposit_type">
              <dx-lookup value-expr="code_name" display-expr="code_name" :data-source="vars.dataSource.deposit_type" />
            </dx-column>
            <dx-column caption="입금적요" data-field="etc" />
            <dx-column caption="미입금" data-field="sales.not_deposit" :visible="false" :allow-editing="false" :show-in-column-chooser="false" />
            <dx-column caption="계산서아이디" data-field="fk_sales_id" :visible="false" :allow-editing="false" :show-in-column-chooser="false" />
    ...
    <dx-popup
      ...
      @initialized="
        (evt) => methods.onGridInitialized(evt, 'find-deposit-popup')
      "
    >
      <dx-toolbar-item
        widget="dxButton"
      ...
      <template #popup-content>
        <dx-data-grid
          ...
          :allow-column-resizing="true"
          :allow-column-reordering="true"
          :data-source="vars.dataSource.sales"
          :on-initialized="evt => methods.onGridInitialized(evt, 'sales')"
        >
          <dx-column caption="고객업체" data-field="client_company" />
          <dx-column caption="계산서번호" data-field="sales_number" :sort-index="1" sort-order="desc" />
          <dx-column caption="발행일자" data-field="sales_date" data-type="date" format="yyyy-MM-dd" :sort-index="0" sort-order="desc" />
          ...
          <dx-column caption="미입금" data-field="not_deposit" data-type="number" :visible="false" :format="{ type: 'fixedPoint', precision: 2 }" />
          ...
          <dx-summary>
            ...
            <dx-total-item column="not_deposit" summary-type="sum" value-format=",##0.00" display-format="미입금: {0}" />
          </dx-summary>
    ...
    >
      <template #popup-content>
        ...
        <data-grid-deposit v-else-if="vars.dlg.finder.key === 'deposit'" @change="methods.finderReturnHandler" />
...
import { shipmentDeposit, getShipmentDepositItem, getShipmentSalesStatement } from '../../data-source/shipment';

import DataGridClient from '../../components/base/data-client.vue';
import DataGridDeposit from '../../components/shipment/data-deposit.vue';
...
export default {
  components: {
    ...
    DataGridClient, DataGridDeposit, DataGridProject,
  },
  props: {
    id: [String, Number],
  },
  setup(props) {
    ...
    vars.grid = { sales: null, item1: null };
    ...
    vars.filter = {
      ...
      item1: [{ name: 'fk_deposit_id', op: 'eq', val: props.id || 0 }],
    };
    ...
    vars.dataSource = reactive({
      ...
      employee: [],
      sales: null,
      item1: getShipmentDepositItem(vars.filter.item1),
    });
    ...
    const methods = {
      async initById(id) {
        ...
        let { data } = await shipmentDeposit.byKey(id);
        Object.assign(vars.formData, data);

        methods.enableDelete();
        vars.disabled.edit = false;
        if (vars.formData.client_company && vars.formData.deposit_department) {
          methods.enableSave();
      ...
      clearFormData() {
        ...
        vars.formData.deposit_number = '';
        vars.formData.deposit_date = '';
        vars.formData.client_company = '';
        vars.formData.client_manager = '';
        vars.formData.deposit_department = '';
        vars.formData.deposit_manager = '';
      ...
      async addSelectedRows() {
        ...
        const rows = await vars.grid.sales.getSelectedRowsData();
        for (let row of rows) {
          await grid.addRow();
          grid.cellValue(0, 'sales.sales_number', row.sales_number); // 계산서번호
          grid.cellValue(0, 'sales.not_deposit', row.not_deposit); // 미입금
          grid.cellValue(0, 'price', row.not_deposit); // 금액
          grid.cellValue(0, 'deposit_type', ''); // 입금형태
          grid.cellValue(0, 'etc', ''); // 입금적요
          grid.cellValue(0, 'fk_sales_id', row.id); // 계산서 아이디
        }
      ...
      showAddPopup() {
        ...
        for (const item of vars.grid.item1.getVisibleRows()) {
          if (!item.id) {
            notContains.push(['id', '<>', item.data.fk_sales_id], 'and');
          }
        }
        notContains.pop()

        if (vars.grid.sales) {
          vars.grid.sales.filter(notContains);
          vars.grid.sales.clearSelection();
          vars.grid.sales.refresh();
        }
      ...
      async newItem() {
        ...
        setTimeout(() => {
          ...
          vars.formData.deposit_date = currentDateTime();
          vars.formData.deposit_department = authService.getDepartmentName();
          vars.formData.deposit_manager = authService.getUserName();
          vars.formData.fk_company_id = authService.getCompanyId();
      ...
      async deleteItem() {
        ...
        if (result) {
          try {
            await shipmentDeposit.remove(vars.formData.id);
      ...
      async saveItem() {
        ...
        try {
          if (vars.formData.id) {
            ...
            delete updateDate.deposit_number;
            const { data } = await shipmentDeposit.update(vars.formData.id, updateDate);
            vars.formData.deposit_number = data.deposit_number;
            ...
            let { data } = await shipmentDeposit.insert(vars.formData);
      ...
      async onClientChanged() {
        ...
        vars.filter.sales.clientCompany = client ? client.name : null;
        methods.loadSales();
      ...
      onDepartmentChanged(e) {
        if (!e.value) {
          ...
          vars.disabled.items = true;
          vars.disabled.manager = true;
          vars.formData.deposit_manager = null;
      ...
      onSavingItem(e) {
        ...
        e.changes.forEach(element => {
          if (element.type != 'remove') {
            element.data.fk_deposit_id = vars.formData.id;
            delete element.data.sales;
          }
        });
        methods.saveSummary();
      },
      saveSummary(){
        ...
        shipmentDeposit.update(vars.formData.id, piceDate)
      ...
      setPrice(newData, value, currentRowData) {
        if (currentRowData.sales && currentRowData.sales.not_deposit) {
          if (value > currentRowData.sales.not_deposit) {
            newData.price = currentRowData.sales.not_deposit;
          } else {
            newData.price = value;
          }
        } else {
      ...
      loadBaseCode() {
        return baseCodeLoader(['입금형태']).then(response => {
          vars.dataSource.deposit_type = response['입금형태'];
        }).then(() => (vars.init.value = true));
      },
      loadSales() {
        vars.dataSource.sales = getShipmentSalesStatement([
          {
            name: 'client_company',
            op: 'eq',
            val: vars.filter.sales.clientCompany,
          },
          {
            name: 'not_deposit',
            op: 'neq',
      ...
      redirect(id) {
        if (id) { router.replace({ path: `/shipment/deposit/${id}` }); }
        else { router.replace({ path: `/shipment/deposit` }); }
      },
      enableDelete() {
        if (vars.formState.readOnly) {
          vars.disabled.delete = true;
