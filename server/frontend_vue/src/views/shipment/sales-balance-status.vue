    ...
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">매출잔액장</div>
            </dx-item>
          </dx-toolbar>
        </div>
      ...
      <div class="dx-card responsive-paddings mt-1">
        ...
        >
          ...
          <dx-column caption="이월 미수금" data-field="past_account_receivable"  data-type="number" format="currency" />
          ...
          <dx-column caption="매출합계" data-field="total_price" data-type="number" format="currency" />
          ...
          <dx-summary>
            ...
            <dx-total-item column="sales_price" show-in-column="sales_price" summary-type="sum" value-format="currency" display-format="{0}" />
            ...
            <dx-total-item column="deposit_price" show-in-column="deposit_price" summary-type="sum" value-format="currency" display-format="{0}" />
  ...
  const params = {}
    const { data: releaseItems } = await shipmentReleaseItem.load({
      filter: [
        ...
        ['release.client_company', '=', detailItem.value.client_name]
      ],
      sort: [
    ...
    const {data : releaseReturnItem } = await shipmentReleaseReturnItem.load({
      filter: [
        ...
        ['release_return.client_company', '=', detailItem.value.client_name]
      ],
      sort: [
    ...
    const { data: depositItems } = await shipmentDepositItem.load({
      filter: [
        ...
        ['deposit.client_company', '=', detailItem.value.client_name]
      ],
      sort: [
  ...
  try {
    ...
    const { data: releaseItems } = await shipmentReleaseItem.load({
      filter: [
        ['release.release_date', '>=', moment(formData.startDate).format('YYYY-MM-DD 00:00:00')],
        ['release.release_date', '<=', moment(formData.endDate).format('YYYY-MM-DD 23:59:59')],
        ['release.client_company', '=', params.item.client_name]
      ],
      sort: [
        { selector: 'release.release_date', desc: false }
      ]
     ...
     const {data : releaseReturnItem } = await shipmentReleaseReturnItem.load({
      filter: [
        ['release_return.return_date', '>=', moment(formData.startDate).format('YYYY-MM-DD 00:00:00')],
        ['release_return.return_date', '<=', moment(formData.endDate).format('YYYY-MM-DD 23:59:59')],
        ['release_return.client_company', '=', params.item.client_name]
      ],
      sort: [
        { selector: 'release_return.return_date', desc: false}
      ]
    ...
    const { data: depositItems } = await shipmentDepositItem.load({
      filter: [
        ['deposit.deposit_date', '>=', moment(formData.startDate).format('YYYY-MM-DD 00:00:00')],
        ['deposit.deposit_date', '<=', moment(formData.endDate).format('YYYY-MM-DD 23:59:59')],
        ['deposit.client_company', '=', params.item.client_name]
      ],
      sort: [
        { selector: 'deposit.deposit_date', desc: false }
      ]
