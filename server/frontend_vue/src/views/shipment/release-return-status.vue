  ...
  <div>
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">출고반품현황</div>
            </dx-item>
          </dx-toolbar>
        </div>
        <div>
          <div class="search-status">
            <span class="search-title">출고반품일자</span>
            ...
            <span class="search-tab"></span>
            <dx-check-box text="미계산서" v-model:value="vars.formData.notRequested" />
      ...
      <div class="dx-card responsive-paddings mt-1">
        ...
        >
          <dx-column caption="반품번호" data-field="release_return.return_number" data-type="string" :filter-operations="['contains', '=']" />
          <dx-column caption="반품일자" data-field="release_return.return_date" data-type="date" format="yyyy-MM-dd" :show-in-column-chooser="false" />
          <dx-column caption="고객업체약칭" data-field="release_return.client_alias" data-type="string" />
          <dx-column caption="고객업체" data-field="release_return.client_company" data-type="string" />
          <dx-column caption="반품부서" data-field="release_return.return_department" data-type="string" />
          <dx-column caption="반품담당자" data-field="release_return.return_manager" data-type="string" />
          <dx-column caption="반품구분" data-field="release_return.return_type" data-type="string" />
          ...
          <dx-column caption="입고창고" data-field="warehouse_code" data-type="string" />
...
import { shipmentReleaseReturnItem } from '../../data-source/shipment';
  ...
  setup() {
    ...
    const methods = {
      ...
      initSorting() {
        ...
        if (col.length == 0) {
          const defaultName = 'release_return.return_date';
      ...
      goDetail({ data }) {
        router.push({ path: `/shipment/release-return/${data.release_return.id}` });
      },
      ...
      getParams () {
        return { 
          ...
          sort: [
            {selector : 'release_return.return_date', desc: true}
          ],
          skip: 0,
          take: 10000
        }
      ...
      async searchDateRange() {
        ...
        // const dateFilter = filter.find(v => typeof v === 'object' && v[0] === 'release_return.return_date')
        // if (dateFilter) dateFilter[2] = filterValue
        // else filter.push(['release_return.return_date', 'between', filterValue])

        // if (vars.formData.notRequested) {
        //   filter.push('and');
        //   filter.push(['non_invoice', '>', 0]);
        // } else {
        //   const index = filter.findIndex(v => typeof v === 'object' && v[0] === 'non_invoice')
        ...
        try{
          ...
          const { data } = await shipmentReleaseReturnItem.load(params);
      ...
      async onExporting(evt) {
        shipmentReleaseReturnItem.exportData(evt.component, '출고반품현황', `출고반품현황_${Date.now()}.xlsx`);
        evt.cancel = true;
      },
    };

    return { vars, methods, shipmentReleaseReturnItem, dataSource };
  },
};
</script>
