    ...
    <div class="content-block">
        ...
        <div class="content-header">
          <dx-toolbar class="back-colored">
            <dx-item location="before">
              <div class="content-title">출고·계산서 현황</div>
            </dx-item>
          </dx-toolbar>
        </div>
        <div>
          <div class="search-status">
            ...
            <dx-check-box text="미계산서유무" v-model:value="vars.formData.nonInvoice" />
            <span class="search-tab"></span>
      ...
      <div class="dx-card responsive-paddings mt-1">
        <dx-data-grid
          ...
          @row-dbl-click="methods.goRelease"
        ...
        >          
          ...
          <template #masterDetailTemplate="{ data: release }">
            <release-to-sales-detail :template-data="release" />
          </template>
        </dx-data-grid>
      </div>
    </div>
...
import ReleaseToSalesDetail from './release-to-sales-detail.vue';
  ...
  setup() {
    ...
    const methods = {
      onGridInitialized(evt, key) {
        ...
        stateStore.bind(`shipment-release-sales-${key}`, evt.component);
      ...
      goRelease(e) {
        router.push({ path: `/shipment/release/${e.data.order.id}` });
      },
      ...
      async searchDateRange() {
        ...
        // const dateFilter = filter.find(v => typeof v === 'object' && v[0] === 'release.release_date')
        // if (dateFilter) dateFilter[2] = filterValue
        // else filter.push(['release.release_date', 'between', filterValue])

        // if (vars.formData.nonInvoice) {
        //   filter.push('and');
      ...
      async onExporting (evt) {
        shipmentReleaseItem.exportData(evt.component, '출고계산서현황', `출고계산서현황_${Date.now()}.xlsx`)
        evt.cancel = true
      }
    };
