  ...
  <div v-if="vars.init.value">
    ...
    <div class="content-block">
      ...
      <div class="dx-card responsive-paddings mt-1">
        <div class="mt-2">
          ...
          >
            <dx-grid-toolbar>
              <dx-grid-item template="receivingAdd" location="after" :visible="vars.control.not_use_prereceiving && !vars.formState.readOnly" />
              <dx-grid-item template="orderAdd" location="after" :visible="!vars.control.not_use_prereceiving && !vars.formState.readOnly" />
              <dx-grid-item name="saveButton" :visible="!!vars.formData.id" />
              <dx-grid-item name="revertButton" />
              <dx-grid-item name="columnChooserButton" />
            </dx-grid-toolbar>
            <template #receivingAdd>
              <dx-button text="가입고에서 가져오기" icon="add" @click="methods.showAddReceivingPopup" />
            </template>
            <template #orderAdd>
              <dx-button text="발주에서 가져오기" icon="add" @click="methods.showAddOrderPopup" />
            </template>
            ...
            <dx-column caption="가입고번호" data-field="prereceiving.prereceiving_number" v-if="vars.control.not_use_prereceiving" />
            ...
            <dx-column caption="프로젝트 번호" data-field="project_management.project_number" :editor-options="{...generateItemButtonOption('search', methods.createFindPopupFn('project-item', '프로젝트조회'))}"
    ...
    >
      <dx-toolbar-item toolbar="top" location="after" widget="dxButton"
        :options="{icon: 'add', text: '선택된 항목 추가', onClick: methods.addOrderItemSelectedRows}"
      />
    ...
    <dx-popup
      title="가입고에서 가져오기"  
      content-template="popup-content"
      v-model:visible="vars.dlg.addPrereceivingItem.show"
      width="70%"
      :height="460"
      :resize-enabled="true"
      :close-on-outside-click="true"
      @initialized="evt => methods.onGridInitialized(evt, 'load-receiving-popup')"
    >
      <dx-toolbar-item toolbar="top" location="after" widget="dxButton"
        :options="{icon: 'add', text: '선택된 항목 추가', onClick: methods.addPrereceivingItemSelectedRows}"
      />
...
import { purchaseReceiving, getPurchaseOrderItem, getPurchaseReceivingItem, getPurchasePreReceivingItem } from '../../data-source/purchase';
...
export default {
  ...
  setup(props) {
    ...
    vars.dlg.addPrereceivingItem = reactive({ show: false });
    ...
    const methods = {
      ...
      async addOrderItemSelectedRows() {
        ...
        for (let row of rows) {
          ...
          data.non_invoice = row.not_shipped; // 미계산서수량
          ...
          data.fk_project_management_id = fk_project_management_id;
          data.fk_purchase_order_id = row.fk_purchase_order_id; // 
      ...
      async addPrereceivingItemSelectedRows() {
        const grid = vars.grid.item1;
        await grid.pageIndex(0);
        const firstRowKey = grid.getKeyByRowIndex(0);
        if (firstRowKey) {
        ...
        for (let row of rows) {
          ...
          data.order_quantity = row.order_item.order_quantity; // 발주수량
          data.receiving_quantity = row.not_shipped; // 입고수량
          data.non_invoice = row.not_shipped; // 미계산서수량
          data.unit_price = row.unit_price; // 단가
          data.supply_price = methods.calculateTotalPriceWithDecimal(data.unit_price, data.receiving_quantity); // 공급가
        ...
        grid.refresh();
        vars.dlg.addPrereceivingItem.show = false;
      },
      ...
      showAddReceivingPopup() {
        ...
        vars.dlg.addPrereceivingItem.show = true;
      },
      ...
      async onClientChanged() {
        ...
        vars.filter.addItem.clientCompany = client ? client.name : null;
      ...
      calculateTotalPriceWithDecimal(unitPrice, quantity) {
        ...
        const total = Math.round(roundedUnitPrice * quantity / conversionUnit);
      ...
      setQuantity(newData, value, currentRowData) {
        ...
        const minQuantity = currentRowData.receiving_quantity - currentRowData.non_invoice;
        const maxQuantity = currentRowData.order_quantity;
        ...
        newData.receiving_quantity = quantity;
        newData.non_invoice = currentRowData.non_invoice + (quantity - currentRowData.receiving_quantity);
        ...
        const filters = [
          {
            name: 'order',
            op: 'has',
            val: { name: 'client_company', op: 'eq', val: vars.filter.addItem.clientCompany }
          },
          { name: 'not_shipped', op: 'gt', val: 0},
          { name: 'check_yn', op: 'eq', val: true }
        ];
        vars.dataSource.orderItem = getPurchaseOrderItem(filters);
      },
