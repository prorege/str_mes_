      ...
      <td>
        <table class="lined-all fixed" style="width:auto;float:right;border:2px solid black;">
          ...
          <tr style="height: 20px;">
            ...
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td style="font-size:10px;padding:0;text-align:center;"></td>
          </tr>
        </table>
        <table class="fixed">
          <colgroup>
            ...
            <% _.forEach(items, function(item, index) { %>
              <tr>
                ...
                <td class="align-right">
                  <%= numeral(item.order_plan_quantity).format('0,0') %>
                  &nbsp;<%= item.item.unit %>
                </td>
                <td class="align-right"><%= numeral(item.unit_price).format('0,0.00') %></td>
                <td class="align-right"><%= numeral(item.supply_price).format('0,0.00') %></td>
                <td>
          ...
          <tfoot>
            <tr>
              ...
              <td colspan="2" class="align-right no-border-x">
                <%= numeral(items.reduce((t, i) => t += parseFloat(i.supply_price), 0)).format('0,0.00') %>
              </td>
...
<div class="bottom-sheet">
  ...
  <div style="display:flex; justify-content: space-between; font-size: 12px;">
    <div>발주계획번호: <%= order_plan_number %> &nbsp;&nbsp;&nbsp; 업체명: <%= client_company %></div>
